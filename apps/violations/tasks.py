...
hdrs = {
    "Origin": "https://portal.columbus.gov",
...
def parse_aspnet_partial_response(response: str) -> list[dict[str, str]]:
    ...
    while i < length:
        ...
        if token == "":
            ...
            results.append({"command": command, "target": target, "data": data})
...
def get_csv(day):
    ...
    form_data["ctl00$PlaceHolderMain$generalSearchForm$txtGSStartDate"] = day.strftime(
        "%m/%d/%Y"
    )  # "05/15/2025"
    form_data["ctl00$PlaceHolderMain$generalSearchForm$txtGSEndDate"] = day.strftime(
        "%m/%d/%Y"
    )  # "05/15/2025"

    resp = sess.post(
        "https://portal.columbus.gov/permits/Cap/CapHome.aspx?module=Enforcement",
        data=form_data,
        headers=hdrs,
    )
    ...
    form_data["ctl00$ScriptManager1"] = (
    ...
    table = sess.get(
        "https://portal.columbus.gov/Permits/Export2CSV.ashx?flag=1248", headers=hdrs
    )
    ...
    latest_scraped_at = (
        CodeViolation.objects.filter(record_number=OuterRef("record_number"))
        .order_by("-scraped_at")
        .values("scraped_at")[:1]
    )
    ...
    latest_qs = CodeViolation.objects.filter(
        record_number__in=record_numbers, scraped_at=Subquery(latest_scraped_at)
    )
