...
def generate_year(year, total_cases=66000):
    ...
    random.shuffle(vals) # argmax always takes the first, accumulating non-zero values towards the end
    print(sum(vals))
    ...
    while day.year == year:
        ...
        cats = random.choices(["CVE","CVF","CVG","CVR"],weights=[629,37087,24018,202], k=no_cases)
        #print(day, no_cases, cats)
        for cat in cats:
            while random.random() < 1-CASE_WORKDAY_RATIO-CASE_WEEKEND_RATIO:
                #print("skip", case_number)
                case_number += 1
            cases.append(f"{year} {cat} {case_number:06d}")
            case_number += 1

        day += timedelta(days=1)
    print(Counter([c[:8] for c in cases]))
